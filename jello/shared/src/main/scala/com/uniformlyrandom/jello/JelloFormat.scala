...
object JelloFormat extends TypesLibrary {
  ...
  /**
    ...
    * @return
    */
  def formatTrait[T](implicit tm: reflect.ClassTag[T]): FormatBuilder[T] =
    FormatBuilder[T]

  def formatSealedTrait[T]: JelloFormat[T] = macro formatSealedTraitImpl[T]

  def formatSealedTraitImpl[T: c.WeakTypeTag](
    ...
    if (!symbol.isClass)
      ...
      val objects =
        companion.typeSignature.decls.filter(_.typeSignature <:< tpe).toList
      val classes = companion.typeSignature.decls
        .filter(_.isClass)
        .filter(_.asClass.baseClasses.contains(symbol))
        .toList
  ...
  def formatEnumeration[E <: Enumeration](enum: E): JelloFormat[E#Value] =
    new JelloFormat[E#Value] {

      override def read(jelloValue: JelloValue): Try[E#Value] =
        jelloValue match {
          case JelloString(s) =>
            try {
              ...
              case _: NoSuchElementException =>
                Failure(
                  new RuntimeException(
                    s"could not find enum member with value [$s]"))
            }

          case _ =>
            Failure(
              new RuntimeException(s"string value expected got [$jelloValue]"))
        }
  ...
  implicit def implicitFormat[A](implicit reader: JelloReader[A],
                                 writer: JelloWriter[A]) =
    JelloFormat[A](reader, writer)

  def apply[A](reader: JelloReader[A], writer: JelloWriter[A]) =
    new JelloFormat[A] {
      override def read(jelloValue: JelloValue): Try[A] =
        reader.read(jelloValue)
  ...
  def formatMacroImpl[T: c.WeakTypeTag](
    ...
    if (tpe.typeSymbol.asClass.isCaseClass) {
      ...
      val classMembers: List[c.universe.Symbol] = ctor.paramLists.head.reverse
      ...
      c.abort(c.enclosingPosition,
              tpe.typeSymbol.fullName + " is not a case class")
    }
